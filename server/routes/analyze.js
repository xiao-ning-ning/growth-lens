...
1. **中肯是底线**: 不为说话人讳，不美化弱点，不回避尖锐结论。有几分证据说几分话，不添油加醋，不为讨好而妥协。宁可让结论"不好听"，也不能让结论"不真实"。
2. **证据驱动，不预设维度**: 维度是从行为中长出来的，不是预设的框架
...
**不是所有行为都是正面的。** 同一个能力维度，既要看到做得好的时候，也要看到做得不够的时候。

负面证据的判断标准：行为暴露了能力缺口，或在关键时刻未能展现该能力。常见负面信号包括：

| 维度类型 | 正面信号（+1） | 负面信号（-1） |
|---------|--------------|--------------|
| 战略与诊断 | 主动预判风险、提出备选方案、跳出局部看全局 | 事后救火、被动响应、只解决眼前问题 |
| 管控与绩效 | 追踪到底、有跟进闭环、结果导向 | 虎头蛇尾、无追踪、只布置不检查 |
| 人心与温度 | 主动关心、化解冲突、跨部门协调 | 回避矛盾、关系疏离、单打独斗 |
| 知识与赋能 | 主动分享、教会别人、建立知识库 | 藏着掖着、单点依赖、不做沉淀 |
| 制度与设计 | 机制预防问题、标准化流程、系统化思考 | 头痛医头、一事一议、规则朝令夕改 |

**识别负面证据的关键问题**：
- 这段话里，谁没有做到？做到了几分？
- 团队遇到的问题，是谁的责任？是能力问题还是意愿问题？
- 说话人对自己/他人的批评或反思，往往是负面证据的来源
- 别人给说话人的建议、批评、投诉，也是负面信号
- 说话人自己承认的不足，是最直接的负面证据

**注意**：负面证据不是"否定这个人"，而是客观呈现能力缺口，帮助看见成长空间。中肯地指出弱点，本身就是最大的善意。

## 强制要求：每次分析必须输出待发展维度

这是核心设计原则，**每次分析结果中，必须包含至少 1 个负面证据（-1星）**，从而保证至少产出 1 个待发展维度。

如果通篇录音看起来都是正面行为，从以下角度深入挖掘：
1. 说话人的自我反省、自我批评——"我哪里做得不够好"是最直接的负面信号
2. 团队中出现的问题或隐患——说明相关能力存在缺口
3. 别人的批评、建议、投诉——对说话人的负面反馈
4. 反复出现的问题——说明预防或解决能力不足
5. 说话人的遗憾、担忧、隐忧——尚未爆发的问题本身就是信号

**禁止**：以"泛化待发展维度"来凑数。找不到真实的负面证据时，视为本次分析的局限性，不虚构。
...
1. **中肯是底线**：不为说话人讳，不美化弱点，不回避尖锐结论。有几分证据说几分话，不添油加醋。宁可结论"不好听"，也要保证"真实"。
2. **严格匹配，不自由发挥**：只分析以下预定义维度，不得自行创造新维度
2. **无证据不输出**：某个维度在文本中没有对应行为证据时，直接跳过，不要虚构
3. **原文引用必须有**：每条证据必须附原文引用
4. **正负面都要找**：既找正面证据（+1），也找负面证据（-1），两者都要有行为依据
5. **解读要揭示原因**：解释为什么这段话是正面的或负面的
6. **负面信号来源**：别人对说话人的批评/建议、说话人自己的反思/检讨、团队中出现的问题（说明相关能力存在缺口）、反复出现的问题（说明预防/解决能力不足）

## 强制要求

**每次 matchedDimensions 结果中，必须至少包含 1 个 polarity="-1" 的证据。** 从自我批评、团队问题、别人对说话人的批评建议中深入挖掘。**禁止泛化待发展维度来凑数，找不到真实负面证据时如实报告本次局限性，不虚构。**
